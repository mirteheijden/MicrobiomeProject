Pre-processing, adapted from @erawijantari, steps:
1. Set working directory: config, DATA, DB, LOGS, RESULTS, SCRIPTS, workflow
...
3.NEXTFLOW
      ...
      Create in Excel, save as .CSV, move to Putty. Database is only real database (MetaPhlan4, mOTU), not the fasta.file documents (human_CHM13)
      Crease the samplesheet: "config/samplesheet.csv"

      Run taxprofiler.sh script for cleaning data + removing human contamination (host contamination):
...
Humann3 output files are in <RESULTS/humann3/raw>. Merge, regroup and renorm: 

1. Merge genefamilies, pathabundance and  pathcoverage files

        $ humann_join_tables --input /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/raw --        file_name genefamilies.tsv  --output /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/merged/genefamilies.txt

        $ humann_join_tables --input /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/raw --file_name pathabundance.tsv  --output /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/merged/pathabundance.txt

        $ humann_join_tables --input /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/raw --file_name pathcoverage.tsv  --output /scratch/project_200XXX/human_feces_batch1/RESULTS/humann3/merged/pathcoverage.txt


2. Create humann_regroup_funct.sh and humann_renorm_rename.sh script and put in the SCRIPTS directory

3. run humann_regroup_funct.sh script

       $ bash ./SCRIPTS/humann_regroup_funct.sh

4. run humann_renorm_rename.sh script

       $ bash ./SCRIPTS/humann_renorm_rename.sh

5. Output files can be found in RESULTS/humann3/final/splitstrat
