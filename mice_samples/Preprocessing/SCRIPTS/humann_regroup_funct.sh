...
#This is the script to regroup the Uniref 90 into various function database Due to small sample size, the process will be done i$
# whole table if samples more than 1000, consider doing the step for each sample individually and merge after due to memmory req$
...
regroup_KO() {
        ...
        DB="/scratch/project_2008347/mice_feces_IV2/DATABASE_DIR/utility_mapping/map_ko_uniref90.txt.gz"
...
regroup_eggnog() {
        ...
        DB="/scratch/project_2008347/mice_feces_IV2/DATABASE_DIR/utility_mapping/map_eggnog_uniref90.txt.gz"
        humann_regroup_table --input "$input" --output "$output" -c $DB
}
...
regroup_GO() {
        ...
        DB="/scratch/project_2008347/mice_feces_IV2/DATABASE_DIR/utility_mapping/map_go_uniref90.txt.gz"
         humann_regroup_table --input "$input" --output "$output" -c $DB
}
...
regroup_pfam() {
        ...
        DB="/scratch/project_2008347/mice_feces_IV2/DATABASE_DIR/utility_mapping/map_pfam_uniref90.txt.gz"
        humann_regroup_table --input "$input" --output "$output" -c $DB
}
